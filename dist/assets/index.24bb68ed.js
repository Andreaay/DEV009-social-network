...
function titleBox2() {
  ...
  const toggleMenu = () => {
    ...
    if (currentPrimaryColor === "rgb(130, 32, 75)") {
      ...
      document.documentElement.style.setProperty("--primary-color", "rgb(130, 32, 75)");
...
function bottomMenu2(navigateTo2, logOutUser2) {
...
class PopupOperation extends AbstractPopupRedirectOperation {
  constructor(auth2, filter, provider2, resolver, user2) {
...
function q$2(a, b2, c) {
  Function.prototype.bind && -1 != Function.prototype.bind.toString().indexOf("native code") ? q$2 = ea : q$2 = fa;
...
function ic(a, b2) {
  ...
  0 < a.O && (a.M = new Ab(q$2(a.Pa, a, a.g), a.O));
      ...
      if (!a.K && xc(c.i, a) && 3 == c.H) {
        ...
        if (Array.isArray(d) && 3 == d.length) {
          ...
          if (0 == e[0])
            ...
            c.Fa = e[1], 0 < c.Fa - c.V && 37500 > e[2] && c.G && 0 == c.A && !c.v && (c.v = Rb(q$2(c.ib, c), 6e3));
          if (1 >= Bc(c.i) && c.oa) {
            try {
              c.oa();
            } catch (m) {
...
k.ha = function(a, b2, c, d) {
  ...
  this.g.onreadystatechange = q$2(this.La, this);
  try {
  ...
  try {
    wd(this), 0 < this.B && ((this.L = xd(this.g)) ? (this.g.timeout = this.B, this.g.ontimeout = q$2(this.ua, this)) : this.A = yb(this.ua, this.B, this)), this.v = true, this.g.send(a), this.v = false;
  } catch (f) {
    vd(this, f);
  }
};
...
function Kd(a, b2, c) {
  ...
  var d = a.h ? q$2(a.h.Va, a.h, a) : null;
  a: {
    var e = a.j;
    let f = -1;
    for (; ; ) {
...
const database$1 = getFirestore(app);
const q$1 = query(collection(database$1, "posts"), orderBy("created_date", "desc"));
const createPost = async (data) => {
  try {
    const docRef = await addDoc(collection(database$1, "posts"), data);
    const likeDocRef = doc(database$1, "posts", docRef.id, "like", data.userId);
...
async function bringPost() {
  console.log("bring function");
  const everyPost = query(collection(database$1, "posts"), orderBy("created_date", "desc"));
  const documents = await getDocs(everyPost);
  return documents;
}
async function updatePost(postId, newData) {
  try {
    const postRef = doc(database$1, "posts", postId);
...
function getProfile() {
  ...
  const userInfo = [user2.displayName, user2.email, user2.photoURL];
  return userInfo;
}
function createProfile(displayName, photoURL) {
  const auth2 = getAuth();
...
function signinUser(email, password, callback) {
  setPersistence(auth, browserSessionPersistence).then(() => signInWithEmailAndPassword(auth, email, password)).then((userCredential) => {
    callback(true);
...
const enterGoogle = () => signInWithPopup(auth, provider);
...
const Register = (navigateTo2) => {
  ...
  registerUser.addEventListener("click", () => {
    addUser(inputEmail.value, inputPassword.value).then((userCredential) => {
      navigateTo2("/start");
    }).catch((error) => {
      error.message;
...
function userLikedPost(postId) {
  ...
  return (_a = likedPostsNew[postId]) == null ? void 0 : _a.includes(userId);
}
const postRender = () => {
  const div = document.createElement("div");
  div.classList = "postReal";
  onSnapshot(q$1, (querySnapshot) => {
    document.querySelector(".postReal").innerHTML = "";
    querySnapshot.forEach((doc2) => {
      const postCard = document.createElement("div");
      postCard.classList = "postCard";
      const postElement = document.createElement("p");
      postElement.textContent = doc2.data().post;
      const numberLikes = document.createElement("span");
      numberLikes.textContent = " " + doc2.data().like + "\u2665";
      const likeButton = document.createElement("button");
      likeButton.innerHTML = '<i class="fa-solid fa-heart"></i>';
      function updateLikeButtonState() {
        if (userLikedPost(doc2.id)) {
      ...
      likeButton.addEventListener("click", () => {
        if (!userLikedPost(doc2.id)) {
          likedPostsNew[doc2.id] = likedPostsNew[doc2.id] || [];
          likedPostsNew[doc2.id].push(getCurrentUser().uid);
          const newData = {
            like: doc2.data().like + 1,
            likeUser: likedPostsNew[doc2.id]
          };
          updatePost(doc2.id, newData);
          updateLikeButtonState();
        }
      });
      ...
      editButton.addEventListener("click", () => {
        function createEditPopup(initialValue) {
          ...
          saveButton2.classList.add("popup-save");
          saveButton2.textContent = "Save";
          const cancelButton2 = document.createElement("button");
          cancelButton2.classList.add("popup-cancel");
          cancelButton2.textContent = "Cancel";
          popup2.appendChild(input);
          popup2.appendChild(saveButton2);
          popup2.appendChild(cancelButton2);
          return popup2;
        }
        const popup = createEditPopup(doc2.data().post);
        ...
        saveButton.addEventListener("click", () => {
          ...
          if (doc2.data().id) {
            updatePost(doc2.data().id, newData);
          }
          popup.remove();
        });
...
const Start = (navigateTo2) => {
  ...
  homeDiv.append(bottomMenu2(navigateTo2));
  homeDiv.append(buttonLogout);
  return homeDiv;
};
getAuth(app);
const database = getFirestore(app);
const q = query(collection(database, "events"), orderBy("created_date", "desc"));
...
const Events = (navigateTo2) => {
  ...
  homeDiv.append(bottomMenu2(navigateTo2, logOutUser));
  return homeDiv;
};
const Newpost = (navigateTo2) => {
  const homeDiv = document.createElement("div");
  ...
  buttonShare.addEventListener("click", () => {
    ...
    if (valuePost.length === 0) {
      ...
      inputPost.value = "";
      createPost(data);
      likedPostsNew2[data.user + data.created_date] = false;
